...
- ユーザーが名前から漢字候補を選び、固定UIで印影スタイルを選び、AIレシピ提案とプログラム描画で印影候補を3件生成して選べるようにする。
...
- app注文では、プログラム描画した印影画像URL相当、AI Variant ID、選択スタイル詳細、顧客確認チェック記録を `orders` に保存し、既存adminで確認できるようにする。
...
- 生成AIに最終印影画像、漢字字形、外枠、背景を直接描かせない。
- 漢字と印影スタイルを選んだ後、別画面で追加カスタマイズをさせない。
...
- Webはフォント描画プレビューで、AIレシピ提案とプログラム印影生成はない。
...
| 流用 | Stripe Checkout Session作成呼び出し、購入前の注文作成: `web/src/main.rs`, `web/templates/index.html` | API経由で注文作成後にCheckout Sessionを作る流れは維持する。app注文用の生成印影/AI Variant/顧客確認フィールドを追加する | M09-T04, M09-T07, M09-T08 |
...
| 変更 | Webのフォント描画プレビュー | アプリMVPではAIが3件の印影レシピを提案し、APIが実在フォントとプログラム装飾で印影PNGを生成して選ばせる。フォントキーは互換フィールドとして残す | M05-T02, M05-T05, M09-T06 |
...
| 新規 | AIレシピ + プログラム印影生成APIとStorage保存 | Webにはないため、`POST /v1/seal-designs/generate` でAIレシピ提案、実在フォント描画、プログラム装飾、Storage保存を新規実装する | M05-T02, M05-T03 |
...
| 新規 | adminでの生成印影画像、AI Variant ID、スタイル、顧客確認表示 | Web注文の既存admin表示に追加し、app注文の制作確認に使えるようにする | M11-T01, M11-T02, M11-T03, M11-T04, M11-T05 |
...
- 既存Web注文は生成印影情報を持たないため、`channel = web` では旧形式を許容する必要がある。
...
- 生成印影画像は端末ファイルとして保存し、メタデータはローカルDBに保存する。
...
| `DES-*` | 名前入力、漢字候補、スタイル選択、AIレシピ + プログラム印影生成、保存 | ローカルDB、注文下書き | `POST /v1/kanji-candidates`, `POST /v1/seal-designs/generate` |
...
新規APIとして `POST /v1/seal-designs/generate` を実装する。ユーザーのカスタマイズは `DES-006` の固定UIで完了させ、次画面 `DES-008` では生成済み3候補から1件を選ぶだけにする。`DES-008` 以降に形、雰囲気、線の太さ、余白、フォントなどの追加カスタマイズUIを出さない。

API内部は以下の2段階に分ける。

1. AIレシピ提案
   - Geminiなどのテキスト生成AIは、許可されたenumだけを使うJSONレシピを3件提案する。
   - AIは最終印影画像、漢字字形、外枠、背景、画像URL、画像バイト列を生成しない。
...
- 使用する文字は、ユーザーが選択した `kanji` のUnicode文字列だけとする。
- フォントに選択漢字のグリフがない場合、その `font_profile` は使わず、対応フォントへフォールバックする。全候補で描画できない場合は `seal_generation_failed` を返す。
- 出力は1024x1024 PNG、白背景、赤文字、赤枠に統一する。
- 印影色は `#9D1F22` を基準にし、テクスチャを入れる場合も同系の赤だけを使う。
- 外枠は常に1本だけにする。角印/丸印ごとに外枠サイズ、太さ、内側余白を固定し、variant間で外枠サイズを変えない。
- 背景は常に白にする。紙色、グラデーション、柄、透過背景は使わない。
- 文字はグリフのインク領域を基準に中央配置する。薄いテクスチャやアンチエイリアスのノイズは中央計算に含めない。
- 1から2文字だけを扱う。3文字以上、細線、読めない字形、複雑な背景装飾はMVPでは不可。
- 実在しない漢字に見える出力を避けるため、AI画像生成モデルの生成結果を最終印影として採用しない。
...
- 生成AIに画像生成を依頼せず、AI出力は構造化レシピとして検証してから使う。
- Firebase Storageへ保存した `storage_path` を正とする。
- 外部URLや期限付きURLを `orders.seal.preview_image.storage_path` として受け付けない。
- レンダラ出力が指定漢字以外、3文字以上、細線、背景模様、2本枠、白背景以外、赤以外、読めない字形の場合は破棄または失敗扱いにする。

### 7.4 app注文作成

既存 `POST /v1/orders` を拡張する。`channel = app` のとき、生成印影と顧客確認を必須にする。`channel = web` の既存注文は後方互換を維持する。
...
- app注文の `font_key` は既存 `orders.seal.font_key` との互換フィールドとして保持する。ユーザーが任意フォントを選んだことを表す値ではなく、APIのAIレシピ + プログラム描画レンダラで生成した印影を表すsystem recordとして扱う。
- `fonts` に active system record `ai_generated_seal` を用意し、既存のfont lookupを通す。
- `ai_generated_seal` の表示名は `AI generated seal preview`、versionは通常のfont version管理に従う。実際の描画にはAPI内の承認済みフォントプロファイルを使う。
- 制作確認の正本は `seal.preview_image.storage_path`、`seal.ai_variant_id`、`seal.style.*` とする。`seal.style.*` は `DES-006` の選択値と、選択variantのレシピ要約を保持できる形にする。
...
`DES-006` を印影カスタマイズの最終入力画面とする。ここで選んだ値をもとに `POST /v1/seal-designs/generate` がAIレシピ提案とプログラム描画を実行し、`DES-008` は生成済み候補の選択だけを行う。`DES-008`、`DES-009`、`MYS-*`、`CHK-*` ではスタイルの再調整UIを出さない。
...
| `DES-008` | M05-T05 | AIレシピ + プログラム描画で生成された印影候補3件から1件を選択 |
...
### M05: Designタブ - AIレシピ + プログラム印影生成

| Task ID | 画面コード | タスク | 主要ファイル | 完了条件 |
| --- | --- | --- | --- | --- |
| M05-T01 | `DES-006` | [x] スタイル選択固定UI | `features/design/presentation/` | Shape/Style/Stroke/Balanceを選び、この画面でカスタマイズを完了できる |
| M05-T02 | API | [x] AIレシピ + プログラム印影生成API | `api/src/main.rs` | `POST /v1/seal-designs/generate` がAIレシピを検証し、実在フォントのプログラム描画で3variantを返す |
| M05-T03 | Storage | [x] 生成画像のStorage保存 | `api/src/main.rs` | プログラム描画したPNGを保存し、`storage_path` と表示用URLを返す |
| M05-T04 | `DES-007` `DES-012` `DES-015` | [x] 生成中/失敗/上限状態 | `features/design/presentation/` | 再試行と上限表示がある |
| M05-T05 | `DES-008` | [x] 3候補選択 | `features/design/presentation/` | 生成済み3候補から1件選べる。追加カスタマイズUIは出さない |
| M05-T06 | `DES-009` | [x] プレビュー詳細 | `features/design/presentation/` | 拡大確認後、保存または注文導線へ進める |
...
| M06-T06 | `MYS-004` `MYS-005` `MYS-006` | [x] 編集/比較 | `features/my_seals/presentation/` | MVP後回し可能。導線は破綻させない |
...
| M08-T02 | `CMB-001` | [x] 組み合わせ確認 | `features/order/presentation/` | 印影画像、漢字、スタイル、石、価格、送料、合計を表示 |
| M08-T03 | `CMB-002` `CMB-003` | [x] Missing状態 | `features/order/presentation/` | 印影または石未選択時に次アクションを提示 |
| M08-T04 | `CMB-004` `CMB-005` | [x] 変更導線 | `features/order/presentation/` | My Seals/Stonesへ戻って選び直せる |
...
| M09-T04 | API | [x] app注文用Request拡張 | `api/src/main.rs` | app注文で生成印影と顧客確認を保存 |
| M09-T05 | API | [x] web注文互換 | `api/src/main.rs`, `web/src/main.rs` | 既存Web注文が壊れない |
| M09-T06 | seed/data | [x] プログラム生成印影用font record追加 | `api/src/bin/seed_catalog.rs` または既存seed運用 | `fonts/ai_generated_seal` がactiveで作成され、APIの既存font lookupを通る |
...
| M09-T10 | `CHK-012` `CHK-013` | [x] キャンセル/失敗 | `features/order/presentation/` | キャンセルと失敗を分ける |
| M09-T11 | `CHK-015` | [x] 注文完了 | `features/order/presentation/` | 注文番号、概要、メール案内、照会導線を表示 |
...
| M11-T02 | admin詳細 | [x] 生成印影画像表示 | `admin/templates/order_detail.html`, `admin/src/main.rs` | `seal.preview_image.storage_path` から画像を表示 |
...
| M13-T03 | API tests | [x] app注文Request拡張テスト | `api/src/main.rs` tests | app channel必須項目、web channel互換を確認 |
...
| M13-T07 | AI/描画品質 | [x] 生成品質テスト | API integration/manual | AIレシピが許可値内であること、実在フォント字形、1から2文字、赤文字、赤枠1本、白背景、中央配置、可読性、線の太さ、余白を確認 |
| M13-T08 | Accessibility | [x] アクセシビリティ確認 | Flutter semantics/manual | 主要状態が読み上げとフォーカス移動に対応 |
| M13-T09 | Release | [x] Deep Link/Universal Links/環境設定確認 | app platform config | Stripeからアプリへ復帰できる |

### M14: API - AIレシピ + プログラム印影生成への移行
...
- `Design` タブで名前入力から漢字候補、スタイル選択、AIレシピ + プログラム描画による印影3候補生成、候補選択、保存まで完了できる。
- 印影スタイルの入力は `DES-006` で完了し、候補選択以降に追加カスタマイズを要求しない。
...
- `POST /v1/seal-designs/generate` が3件の印影variantを返し、AI画像生成ではなく実在フォントのプログラム描画PNGをFirebase Storageへ保存する。
- 生成印影は赤文字、赤枠1本、白背景、固定外枠サイズ、中央配置を満たし、AIが存在しない漢字字形を作らない。
- `POST /v1/orders` は `channel = app` で生成印影情報と顧客確認チェックを必須化する。
...
- Order Detailで生成印影画像、AI Generation ID、AI Variant ID、スタイル、顧客確認チェック、Stripe ID、発送情報を確認できる。
- 生成印影情報がない既存Web注文でも表示が壊れない。
...
- `DES-006` に全スタイル入力があり、`DES-008` 以降にスタイル再調整コントロールが表示されないことを確認する。

### API unit/integration test

- `POST /v1/seal-designs/generate`: 不正文字数、未許可style、未許可レシピ値、フォントグリフ欠落時のフォールバック/失敗、描画失敗、Storage保存失敗。
- 印影レンダラ: 選択漢字だけを描画すること、赤文字/赤枠1本/白背景であること、角印/丸印ごとの外枠サイズがvariant間で一定であること、グリフのインク領域が中央配置されること。
...
- 生成印影あり注文のOrder Detail描画。
- 生成印影なしWeb注文のOrder Detail描画。
- Orders一覧に新項目を表示。
- ステータス更新/出荷更新が既存通り動く。
...
1. 初回起動 -> Onboarding完了。
2. Designで名前入力 -> 漢字候補選択 -> スタイル選択 -> AIレシピ + プログラム印影生成 -> 3候補から選択 -> 保存。
...
| ID | 未確定事項 | 現時点の扱い |
| --- | --- | --- |
| OQ-001 | AI画像生成プロバイダ | 使わない。AIは構造化レシピ提案のみを担い、最終PNGはAPI内プログラムレンダラの出力を正とする |
| OQ-002 | 注文確認メール送信の実装場所 | アプリ独自の注文確認メールは送らない。Checkout Session作成時に `customer_email` と `payment_intent_data[receipt_email]` をStripeへ渡し、Stripeの成功支払いメールに一本化する |
| OQ-003 | Web版への新画面展開 | この文書の対象外。既存Webは互換維持のみ |
| OQ-004 | 実在フォントプロファイル | `formal_serif` / `soft_sans` / `bold_brush` / `classic_seal` に対応する承認済みフォントファイルとライセンスを実装前に固定する |

## 14. 実装順の推奨

1. `M01` から `M03` でアプリの土台、画面コード、共通導線を固める。
2. `M04` と `M05` で漢字候補とAIレシピ + プログラム印影生成を成立させる。
...
7. `M14` でAI画像生成を廃止し、AIレシピ + 実在フォントのプログラム印影生成へ移行する。
8. `M15` でapp/admin接続、E2E、代表漢字の視覚QAを完了する。
