...
| `photos[].storage_path` | string | Yes | Storage パス（例: `materials/jade/mat_xxxxx.webp`） |
...
| `price_by_currency` | map<string,int> | Yes | 通貨別単価（例: `USD -> 3600`, `JPY -> 5200`） |
| `is_active` | bool | Yes | 選択可能フラグ |
| `sort_order` | int | Yes | 表示順（昇順） |
...
| `material.unit_price` | int | Yes | 注文時点の単価（`pricing.currency` の最小単位） |
...
- 管理画面 API は編集用途のため `materials` / `countries` の `*_i18n` をそのまま返す。
- `materials.photos[].alt_i18n` も同じフォールバック規則で解決する。
- 注文時は `orders.locale` と `contact.preferred_locale` を保持し、メール/領収書/管理画面表示言語に利用する。
- 受注後のマスタ翻訳更新の影響を避けるため、注文には `material.label_i18n` / `shipping.country_label_i18n` と `seal.font_label` をスナップショット保存する。
...
- 翻訳運用: `materials` / `countries` の `*_i18n` に欠損がある場合は `default_locale` へフォールバックし、欠損メトリクスを記録する。
- 画像運用: 原本と表示用サイズ（サムネイル等）を Storage に保持し、`photos[].storage_path` は表示用途ごとに管理する。

## 11. 移行手順（モック設計から）
1. `materials` / `countries` の `label` 系を `*_i18n` へ移行する（`ja` 値は既存文字列を移植）。`fonts` は `label`（単一文字列）を維持し、`materials` は `photos` フィールドを追加する。
2. `app_config/public` を作成し、`supported_locales=["ja","en"]`, `default_locale="ja"`, `default_currency="USD"`, `currency_by_locale={"ja":"JPY","en":"USD"}` を投入する。
3. `orders` を本番スキーマ（`locale`, `seal.font_label`, `material.label_i18n`, `shipping.country_label_i18n`, `payment.*`, `fulfillment.*`, `idempotency_key` など）へ切り替える。
