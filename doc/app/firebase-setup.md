...
## Flavor mapping
- Flavors: `dev`, `prod`
- Android `applicationId`: `org.finitefield.hanko` (dev/prod share the same in this repo)
- iOS bundle ID: `org.finitefield.hanko` (dev/prod share the same in this repo)
...
## Native config files
- Android: if you use distinct package names per flavor, place `google-services.json` under `android/app/src/dev` and `android/app/src/prod` to match product flavors. (This repo currently uses the same `applicationId` for dev/prod.)
- iOS: add a scheme/xcconfig per flavor that points to the correct `GoogleService-Info.plist` (e.g., `Runner-dev`, `Runner`). Ensure the plist bundle IDs match the IDs above.
...
- Dev: `flutter run --flavor dev --dart-define=APP_FLAVOR=dev`
- Prod: `flutter run --flavor prod --dart-define=APP_FLAVOR=prod`
