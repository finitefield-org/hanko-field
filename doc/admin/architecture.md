...
- **Assets**: Tailwind compiled with the Tailwind CLI (installed via npm) invoked via Make targets. Generated CSS is served from `/public/static/`. Optional Alpine.js (or similar) can ship as additional static files.
...
2. `make css` (or `make dev`) runs the Tailwind CLI to emit `public/static/app.css` (hashed filenames can be added later by the build).
