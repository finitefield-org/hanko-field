...
// ResponsiveSrcset produces a srcset string for an image by appending width query params.
func ResponsiveSrcset(src string, widths ...int) string {
	...
	for _, w := range ws {
		...
		parts = append(parts, widthDescriptor(src, w))
	}
...
func widthDescriptor(src string, width int) string {
	if strings.HasPrefix(src, "data:") {
		return src + " " + strconv.Itoa(width) + "w"
	}
	u, err := url.Parse(src)
	if err != nil {
		return src + " " + strconv.Itoa(width) + "w"
