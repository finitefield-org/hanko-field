...
// AccountBillingHandler renders the account billing page.
func AccountBillingHandler(w http.ResponseWriter, r *http.Request) {
	lang := mw.Lang(r)
	sess := mw.GetSession(r)

	view := buildAccountBillingPageView(lang, sess)

	title := i18nOrDefault(lang, "account.billing.seo.title", "Account · Billing")
	desc := i18nOrDefault(lang, "account.billing.seo.desc", "Review invoices, payment history, and workspace usage.")
	...
	renderPage(w, r, "account_billing", vm)
}
