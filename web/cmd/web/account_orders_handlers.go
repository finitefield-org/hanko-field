...
// AccountOrdersHandler renders the account orders listing page.
func AccountOrdersHandler(w http.ResponseWriter, r *http.Request) {
	lang := mw.Lang(r)
	sess := mw.GetSession(r)

	view := buildAccountOrdersPageView(lang, sess, r.URL.Query())

	title := i18nOrDefault(lang, "account.orders.seo.title", "Account · Orders")
	desc := i18nOrDefault(lang, "account.orders.seo.desc", "Review historical orders, monitor fulfillment stages, and download invoices.")
	...
	renderPage(w, r, "account_orders", vm)
}

// AccountOrdersTableHandler renders the orders table fragment (htmx).
func AccountOrdersTableHandler(w http.ResponseWriter, r *http.Request) {
	lang := mw.Lang(r)
	sess := mw.GetSession(r)

	view := buildAccountOrdersPageView(lang, sess, r.URL.Query())
	renderTemplate(w, r, "frag_account_orders_table", view.Table)
}
